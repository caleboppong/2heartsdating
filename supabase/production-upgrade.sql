-- 2heartsdating production upgrade SQL
...
create table if not exists public.preferences (
  ...
  preferred_gender text, age_min int, age_max int,
  preferred_religion text, preferred_denomination text,
  preferred_profession text, preferred_country text, preferred_city text,
...
create table if not exists public.subscriptions (
  ...
  plan text default 'free' check (plan in ('free','premium','gold','platinum')),
...
drop policy if exists "Approved photos are public" on public.profile_photos;
drop policy if exists "Users upload own photos" on public.profile_photos;
drop policy if exists "Users update own photos" on public.profile_photos;
...
drop policy if exists "Users create their matches" on public.matches;
drop policy if exists "Users see match messages" on public.messages;
drop policy if exists "Users send match messages" on public.messages;
drop policy if exists "Users update read status for match messages" on public.messages;
...
create policy "Users create their matches" on public.matches for insert with check (user1_id = auth.uid() or user2_id = auth.uid() or public.is_admin());
...
create policy "Users update read status for match messages" on public.messages for update using (public.is_admin() or exists(select 1 from public.matches m where m.id = match_id and (m.user1_id = auth.uid() or m.user2_id = auth.uid())));
...
-- Make Caleb admin after he has signed up and saved a profile
update public.profiles set role = 'admin' where id = (select id from auth.users where email = 'maroppcal@hotmail.it');
